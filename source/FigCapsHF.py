...
class FigCapsHF():
    ...
    def generate_jsonl(self):
        """
        Generates metadata.jsonl's for train/test/val and places it in their respective folders in No-Subfig-Img
        """
        ...
        for image_name in tqdm(file_idx):
            ...
            captions_list.append({"file_name":file_metadata['figure-ID'],"text":file_metadata['1-lowercase-and-token-and-remove-figure-index']['caption']})
            c.close()
        f.close()
        ...
        with open(test_image_folder_path + "metadata.jsonl", 'w') as f:
            for caption in captions_list:
                f.write(json.dumps(caption) + "\n")
        f.close()
        print("Done generating jsonl for train, test and validation")
    ...
    def get_image_caption_pair(self, data_split, image_name):
        ...
        data = json.load(f)
        print("The caption is: " + data['1-lowercase-and-token-and-remove-figure-index']['caption'])
        f.close()
    ...
    def get_image_caption_pair_hf(self, image_name):
        ...
        if data_split!="na" and len(data_row)!=0:
            ...
            print(f"The associated human feedback for this image is - helpfulness: {data_row[0][7]}, ocr: {data_row[0][8]}, visual: {data_row[0][9]}, takeaway: {data_row[0][10]}")
    
    def generate_embeddings(self, image_paths, captions_list, model_name, MCSE_Path =  None):
        """
        ...
        if model_name == "SciBERT" or model_name == "BERT" or model_name == "MCSE":
            if model_name == "MCSE":
                model_name = MCSE_Path
            else:
    ...
    def generate_embeddings_hf_anno(self,hf_score_type, model_name, MCSE_path=None):
        ...
        hf_ds_embeddings = self.generate_embeddings(hf_captions, hf_image_paths, model_name, MCSE_Path =  MCSE_path)
    ...
    def generate_embeddings_ds_split(self, model_name, MCSE_path=None, split_name = "train", max_num_samples = None):
        ...
        #Generate embeddings for the larger training set (TODO)
        #CHECK FOR JSONL
        # self.generate_jsonl()
        ...
        ds_split_embeddings = self.generate_embeddings(captions, image_paths, model_name, MCSE_Path = MCSE_path)
        return (ds_split_embeddings, image_names, captions)
    
    def infer_hf_training_set(self, hf_score_type, model_name, MCSE_path = None, max_num_samples = None, quantization_levels = 2, mapped_hf_labels = ["bad", "good"]):
        ...
        hf_ds_embeddings, scores = self.generate_embeddings_hf_anno(hf_score_type,model_name, MCSE_path = MCSE_path)
        scoring_model = self.train_scoring_model(hf_ds_embeddings,scores)
        train_ds_embeddings, image_names, training_captions  = self.generate_embeddings_ds_split(model_name, MCSE_path, "train", max_num_samples)
